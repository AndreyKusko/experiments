...
__get_response = functools.partial(request_response_delete, path="/api/v1/worker-instructions/")


@pytest.mark.parametrize("instance", [pytest.lazy_fixture("worker_instruction_fi")])
...
def test__accepted_owner__success(api_client, mock_policies_false, r_cu, get_worker_instruction_fi):
    instance = get_worker_instruction_fi(company=r_cu.company)
    assert WorkerInstruction.objects.filter(project__company=r_cu.company).exists()
    assert __get_response(api_client, instance_id=instance.id, user=r_cu.user)
    assert not WorkerInstruction.objects.filter(project__company=r_cu.company).existing().exists()


@pytest.mark.parametrize("r_cu", [pytest.lazy_fixture("cu_manager_fi")])
def test__related_manager__with_policy__success(api_client, monkeypatch, r_cu, get_worker_instruction_fi):
    monkeypatch.setattr(Policies, "has_object_policy", lambda *a, **kw: True)
    monkeypatch.setattr(Policies, "get_target_policies", lambda *a, **kw: [r_cu.company.id])

    instance = get_worker_instruction_fi(company=r_cu.company)
    assert WorkerInstruction.objects.filter(project__company=r_cu.company).existing().exists()
    assert __get_response(api_client, instance_id=instance.id, user=r_cu.user)
    assert not WorkerInstruction.objects.filter(project__company=r_cu.company).existing().exists()
...
def test__related_manager__without_policy__fail(
    api_client, mock_policies_false, r_cu, get_worker_instruction_fi, get_project_fi
):
    project = get_project_fi(company=r_cu.company)
    instance = get_worker_instruction_fi(project=project)

    assert WorkerInstruction.objects.filter(project=project).existing().exists()
...
def test__not_accept_user__fail(api_client, monkeypatch, get_cu_fi, get_worker_instruction_fi, role, status):
    ...
    instance = get_worker_instruction_fi(company=company)
...
def test__different_company_user__fail(
    api_client, monkeypatch, get_cu_fi, get_worker_instruction_fi, role, status
):
    r_cu = get_cu_fi(status=status.value, role=role)
    instance = get_worker_instruction_fi()
