...
def __get_response(
    ...
    response = api_client.get(f"/api/v1/media-permissions/{PROJECT_QUESTIONNAIRE}/{model_id}/{object_id}/")
...
def test__accpeted_owner__success(api_client, mock_policies_false, r_u, get_project_questionnaire_fi):
...
@pytest.mark.parametrize("r_u", [pytest.lazy_fixture("user_fi")])
def test__familiar__success(monkeypatch, api_client, r_u, get_project_questionnaire_fi):
    object_id = "e0kj309230e0203"
    media_field = {TYPE: PHOTO, VALUES: [object_id], BASENAME: "123123123.jpeg"}
    parent_instance = get_project_questionnaire_fi()
    parent_instance.media_json = [media_field]
    parent_instance.save()
...
@pytest.mark.parametrize("r_u", [pytest.lazy_fixture("user_fi")])
def test__401__fail(monkeypatch, api_client, r_u, get_project_questionnaire_fi):
    object_id = "e0kj309230e0203"
    media_field = {TYPE: PHOTO, VALUES: [object_id], BASENAME: "123123123.jpeg"}
    parent_instance = get_project_questionnaire_fi(user=r_u)
    parent_instance.media_json = [media_field]
    ...
    response = __get_response(api_client, model_id=parent_instance.id, object_id=object_id)
    assert not response.data
