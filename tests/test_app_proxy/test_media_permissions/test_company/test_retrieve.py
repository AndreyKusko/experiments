...
def __get_response(
    ...
    response = api_client.get(f"/api/v1/media-permissions/{COMPANY}/{model_id}/{object_id}/")
...
def test__accepted_owner__success(api_client, mock_policies_false, r_cu, get_company_fi):
...
@pytest.mark.parametrize("r_cu", [pytest.lazy_fixture("cu_owner_fi")])
def test__familiar__success(monkeypatch, api_client, r_cu, get_company_fi):
    object_id = "e0kj309230e0203"
    media_field = {TYPE: PHOTO, VALUES: [object_id], BASENAME: "123123123.jpeg"}
    parent_instance = get_company_fi()
    parent_instance.logo = media_field
...
@pytest.mark.parametrize("r_cu", [pytest.lazy_fixture("cu_owner_fi")])
def test__401__fail(monkeypatch, api_client, r_cu, get_company_fi):
    object_id = "e0kj309230e0203"
    media_field = {TYPE: PHOTO, VALUES: [object_id], BASENAME: "123123123.jpeg"}
    parent_instance = get_company_fi()
    parent_instance.logo = media_field
