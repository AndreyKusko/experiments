...
from tests.test_app_background.test_export.test_processed_reports.constants import PATH, TASK_TYPE, MODEL_NAME
...
def test__accepted_owner__success(
    api_client, mock_policies_false, monkeypatch, r_cu, new_background_task_data, get_processed_report_fi
):
...
def test__not_accepted_owner__fail(
    api_client, monkeypatch, get_cu_owner_fi, new_background_task_data, get_processed_report_fi, status
):
    monkeypatch.setattr(export_processed_reports_task, "delay", lambda *a, **k: None)
...
def test__accepted_manager_with_policy__success(
    api_client, monkeypatch, r_cu, new_background_task_data, get_processed_report_fi
):
    monkeypatch.setattr(export_processed_reports_task, "delay", lambda *a, **k: None)
    monkeypatch.setattr(Policies, "has_object_policy", lambda *a, **k: True)
...
def test__accepted_manager_without_policy__fail(
    ...
    monkeypatch.setattr(export_processed_reports_task, "delay", lambda *a, **k: None)
    monkeypatch.setattr(Policies, "has_object_policy", lambda *a, **k: False)
...
def test__not_accepted_manager_with_policy__fail(
    api_client, monkeypatch, get_cu_manager_fi, new_background_task_data, get_processed_report_fi, status
):
    monkeypatch.setattr(export_processed_reports_task, "delay", lambda *a, **k: None)
...
def test__if_another_company__fail(
    ...
    export_instance = get_processed_report_fi()
    assert not BackgroundTask.objects.filter(company=r_cu.company, task_type=TASK_TYPE).exists()
...
def test__different_reports_schemes__fail(
    ...
    export_instance1 = get_processed_report_fi(project_territory=pt)
    export_instance2 = get_processed_report_fi(project_territory=pt)
    assert not BackgroundTask.objects.filter(company=r_cu.company, task_type=TASK_TYPE).exists()
