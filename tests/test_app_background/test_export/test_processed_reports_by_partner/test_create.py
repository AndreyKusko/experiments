...
from background.serializers.for_export.processed_reports import (
    ExportProcessedReportsSerializer,
)
from tests.test_app_background.test_export.test_processed_reports_by_partner.constants import (
    PATH,
    TASK_TYPE,
    MODEL_NAME,
)
...
def test__accepted_owner__success(
    ...
    get_project_fi,
    get_active_company_partnership_fi,
    get_active_project_partnership_fi,
    get_project_scheme_partnership_fi,
    get_processed_report_form_fi,
):
    monkeypatch.setattr(export_processed_reports_task, "delay", lambda *a, **k: None)

    cp = get_active_company_partnership_fi(invited_company=r_cu.company)
    project = get_project_fi(company=cp.inviting_company)
    pp = get_active_project_partnership_fi(company_partnership=cp, project=project)
    assert pp.partner_company == r_cu.company
    ...
    export_instance = get_processed_report_fi(processed_report_form=prf)
...
@pytest.mark.parametrize("r_cu", [pytest.lazy_fixture("cu_manager_fi")])
def test__if_another_company__fail(
    api_client,
    monkeypatch,
    r_cu,
    new_background_task_data,
    ...
    params = {BTParamsKeys.IDS: [export_instance.id], BTParamsKeys.OUTPUT_FILE_TYPE: BTFT.xlsx.value}
    data = new_background_task_data(params=params)
    response = __get_response(api_client, data=data, user=r_cu.user, status_codes=PermissionDenied)
    assert response.data == {"detail": REQUESTING_USER_NOT_BELONG_TO_INSTANCE_COMPANY}


@pytest.mark.parametrize("r_cu", [pytest.lazy_fixture("cu_manager_fi")])
def test__different_reports_schemes__fail(
    api_client,
    monkeypatch,
    r_cu,
    new_background_task_data,
    ...
    pp = get_active_project_partnership_fi(company_partnership=cp, project=project)
    assert pp.partner_company == r_cu.company
    psp1 = get_project_scheme_partnership_fi(project_partnership=pp)
    psp2 = get_project_scheme_partnership_fi(project_partnership=pp)
    prf1 = get_processed_report_form_fi(project_scheme=psp1.project_scheme)
    prf2 = get_processed_report_form_fi(project_scheme=psp2.project_scheme)
    assert not BackgroundTask.objects.filter(company=r_cu.company, task_type=TASK_TYPE).exists()

    export_instance1 = get_processed_report_fi(processed_report_form=prf1)
    export_instance2 = get_processed_report_fi(processed_report_form=prf2)
    assert not BackgroundTask.objects.filter(company=r_cu.company, task_type=TASK_TYPE).exists()

    params = {
        BTParamsKeys.IDS: [export_instance1.id, export_instance2.id],
    ...
    assert response.data == {"non_field_errors": [MANAGER_REPORTS_DO_NOT_BELONG_TO_SAME_PS]}
    assert not BackgroundTask.objects.filter(company=r_cu.company, task_type=TASK_TYPE).exists()
    ...
    [
        ...
        (
            ...
            OUTPUT_FILE_TYPE_NOT_ALLOWED.format(
                file_types=", ".join([str(v) for v in ExportProcessedReportsSerializer._file_types]),
