...
from background.serializers.for_export.processed_reports import ExportProcessedReportsSerializer
from tests.test_app_background.test_export.test_schedule_time_slots.constants import (
    PATH,
    TASK_TYPE,
    MODEL_NAME,
)
...
def test__accepted_owner__success(
    api_client, mock_policies_false, monkeypatch, r_cu, new_background_task_data, get_schedule_time_slot_fi
):
    monkeypatch.setattr(export_schedule_time_slots_task, "delay", lambda *a, **k: None)
...
def test__not_accepted_owner__fail(
    api_client, monkeypatch, get_cu_owner_fi, new_background_task_data, get_schedule_time_slot_fi, status
):
    monkeypatch.setattr(export_schedule_time_slots_task, "delay", lambda *a, **k: None)
...
def test__accepted_manager_with_policy__success(
    api_client, monkeypatch, r_cu, new_background_task_data, get_schedule_time_slot_fi
):
    monkeypatch.setattr(export_schedule_time_slots_task, "delay", lambda *a, **k: None)
    monkeypatch.setattr(Policies, "has_object_policy", lambda *a, **k: True)
...
def test__not_accepted_manager_with_policy__fail(
    api_client, monkeypatch, get_cu_manager_fi, new_background_task_data, get_schedule_time_slot_fi, status
):
    monkeypatch.setattr(export_schedule_time_slots_task, "delay", lambda *a, **k: None)
...
def test__if_another_company__fail(
    ...
    export_instance = get_schedule_time_slot_fi()
    assert not BackgroundTask.objects.filter(company=r_cu.company, task_type=TASK_TYPE).exists()
