...
__get_response = functools.partial(request_response_create, path="/api/v1/project-variable-values/")
...
def test__response_data(api_client, monkeypatch, r_cu, new_project_variable_value_data):
    monkeypatch.setattr(Policies, "has_object_policy", lambda *a, **kw: True)
    monkeypatch.setattr(requests, "get", MockResponse, raising=True)

    data = new_project_variable_value_data(company=r_cu.company)
    ...
    created_instance = ProjectVariableValue.objects.get(id=created_instance_id)

    response_project_variable = response_instance.pop("project_variable")
    assert response_project_variable == {
        "id": created_instance.project_variable.id,
        "key": created_instance.project_variable.key,
        "kind": created_instance.project_variable.kind,
    }

    assert response_instance.pop("model_id") == created_instance.model_id == data["model_id"]
    assert response_instance.pop("model_name") == created_instance.model_name == data["model_name"]
    assert response_instance.pop("value") == created_instance.value == data["value"]
    assert not response_instance


@pytest.mark.parametrize("r_cu", [pytest.lazy_fixture("cu_owner_fi")])
def test__accepted_owner__success(api_client, mock_policies_false, r_cu, new_project_variable_value_data):
    data = new_project_variable_value_data(company=r_cu.company)
    response = __get_response(api_client, data=data, user=r_cu.user)
    assert ProjectVariableValue.objects.filter(id=response.data["id"]).exists()
    assert response.data.get("id")


@pytest.mark.parametrize("status", NOT_ACCEPT_CUS)
def test__not_accepted_owner__fail(
    api_client, monkeypatch, get_cu_owner_fi, new_project_variable_value_data, status
):
    monkeypatch.setattr(Policies, "has_object_policy", lambda *a, **kw: True)
...
def test__not_owner__without_policy__fail(
    api_client, mock_policies_false, get_cu_fi, new_project_variable_value_data, role
):
...
def test__not_owner__with_policy__success(
    api_client, monkeypatch, get_cu_fi, new_project_variable_value_data, role
):
    monkeypatch.setattr(Policies, "has_object_policy", lambda *a, **kw: True)
...
def test__not_ta__cu__fail(
    ...
    monkeypatch,
    r_cu,
    new_project_variable_value_data,
    has_object_policy,
    is_user,
    field,
    err_text,
...
@pytest.mark.parametrize("r_cu", [pytest.lazy_fixture("cu_owner_fi")])
def test__deleted_company__fail(api_client, monkeypatch, r_cu, new_project_variable_value_data):
    monkeypatch.setattr(Policies, "has_object_policy", lambda *a, **kw: True)
