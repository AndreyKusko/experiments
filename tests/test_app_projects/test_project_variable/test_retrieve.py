...
__get_response = functools.partial(request_response_get, path="/api/v1/project-variables/")
...
def test__response_data(api_client, monkeypatch, r_cu, get_project_variable_fi):
    ...
    assert response_instance.pop("key") == instance.key
    assert response_instance.pop("kind") == instance.kind
...
def test__related_pt_worker__without_policy__without_reservation__fail(
    api_client, mock_policies_false, r_cu, get_pt_worker_fi, get_project_variable_fi
):
    ptw = get_pt_worker_fi(company_user=r_cu)

    instance = get_project_variable_fi(project=ptw.project_territory.project)
...
@pytest.mark.parametrize("r_cu", [pytest.lazy_fixture("cu_owner_fi")])
def test__deleted_cu__fail(api_client, monkeypatch, mock_policies_false, r_cu, get_project_variable_fi):
