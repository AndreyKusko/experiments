...
__get_response = functools.partial(request_response_list, path="/api/v1/project-variables/")
...
def test__user_without_companies__fail(api_client, monkeypatch, user_fi: User):
...
def test__accepted_owner__without_policies__success(
    api_client, mock_policies_false, get_project_variable_fi, r_cu, qty
):

    [get_project_variable_fi(company=r_cu.company) for _ in range(qty)]
...
@pytest.mark.parametrize("r_cu", [pytest.lazy_fixture("cu_owner_fi")])
@pytest.mark.parametrize("qty", (1, 3))
def test__response_data(api_client, monkeypatch, get_project_variable_fi, r_cu, qty):
    monkeypatch.setattr(Policies, "has_object_policy", lambda *a, **kw: False)
    monkeypatch.setattr(Policies, "get_target_policies", lambda *a, **kw: [])
    monkeypatch.setattr(Policies, "list", lambda *a, **kw: [])

    instances = [get_project_variable_fi(company=r_cu.company) for _ in range(qty)]
    instances.reverse()
    ...
    for index, response_instance in enumerate(response.data):
        ...
        assert response_instance.pop("project") == {"id": instances[index].project.id}

        assert response_instance.pop("title") == instances[index].title
        assert response_instance.pop("key") == instances[index].key
        assert response_instance.pop("kind") == instances[index].kind
        assert not response_instance


@pytest.mark.parametrize("status", NOT_ACCEPT_CUS)
@pytest.mark.parametrize("qty", (3,))
def test__not_accepted_owner__fail(api_client, monkeypatch, get_project_variable_fi, get_cu_fi, status, qty):
...
def test__not_owner__without_policies__fail(
    api_client, mock_policies_false, get_project_variable_fi, get_cu_fi, role, qty
):
...
def test__not_owner__with_policies__success(
    api_client, monkeypatch, get_project_variable_fi, get_cu_fi, role, qty
):
    r_cu = get_cu_fi(role=role, status=CUS.ACCEPT.value)
...
def test__not_ta__cu__fail(
    ...
    get_project_variable_fi(company=r_cu.company)
