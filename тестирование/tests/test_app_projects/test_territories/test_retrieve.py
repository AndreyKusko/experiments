...
__get_response = functools.partial(request_response_get, path="/api/v1/territories/")


@pytest.mark.parametrize("instance", [pytest.lazy_fixture("territory_fi")])
...
def test__accepted_related_user__success(api_client, get_cu_fi, get_territory_fi: Callable, role):
...
def test__not_accepted_related_user__fail(api_client, get_cu_fi, get_territory_fi: Callable, status, role):
...
def test__not_related_user__fail(api_client, r_cu: CompanyUser, get_territory_fi: Callable):
