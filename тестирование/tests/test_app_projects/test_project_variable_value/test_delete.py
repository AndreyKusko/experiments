...
__get_response = functools.partial(request_response_delete, path="/api/v1/project-variable-values/")
...
def test__accepted_owner__success(api_client, mock_policies_false, r_cu, get_project_variable_value_fi):

    instance = get_project_variable_value_fi(company=r_cu.company)
    assert ProjectVariableValue.objects.filter(project_variable__project__company=r_cu.company).exists()
    assert __get_response(api_client, instance_id=instance.id, user=r_cu.user)
    assert (
        not ProjectVariableValue.objects.filter(project_variable__project__company=r_cu.company)
        .existing()
        .exists()
    )
...
def test__related__manager__with_policy__success(
    api_client, monkeypatch, r_cu, get_project_variable_value_fi
):
    ...
    instance = get_project_variable_value_fi(company=r_cu.company)
    filter_data = dict(project_variable__project__company=r_cu.company)
    assert ProjectVariableValue.objects.filter(**filter_data).exists()
    assert __get_response(api_client, instance_id=instance.id, user=r_cu.user)
    assert not ProjectVariableValue.objects.filter(**filter_data).existing().exists()


@pytest.mark.parametrize("r_cu", [pytest.lazy_fixture("cu_manager_fi")])
def test__related__manager__without_policy__fail(
    api_client, mock_policies_false, r_cu, get_project_variable_value_fi, get_project_fi
):

    project = get_project_fi(company=r_cu.company)
    instance = get_project_variable_value_fi(project=project)
    assert ProjectVariableValue.objects.filter(project_variable__project=project).exists()
    assert __get_response(api_client, instance.id, r_cu.user, status_codes=NotFound)
    assert ProjectVariableValue.objects.filter(project_variable__project=project).existing().exists()


@pytest.mark.parametrize("role", ROLES_WITH_DIFFERENT_LOGIC)
@pytest.mark.parametrize("status", NOT_ACCEPT_CUS)
def test__not_accept_user__fail(
    api_client, monkeypatch, get_cu_fi, get_project_variable_value_fi, role, status
):
    ...
    instance = get_project_variable_value_fi(company=company)
...
def test__different_company_user__fail(
    ...
    r_cu = get_cu_fi(status=status.value, role=role)
    instance = get_project_variable_value_fi()
    company = instance.project_variable.project.company
