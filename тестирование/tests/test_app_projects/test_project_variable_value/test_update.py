...
__get_response = functools.partial(request_response_update, path="/api/v1/project-variable-values/")
...
def test__accepted_owner__without_policy__success(
    api_client, mock_policies_false, r_cu, get_project_variable_value_fi
):
...
def test__accepted__not_owner__without_policy__fail(
    api_client, mock_policies_false, get_cu_fi, get_project_variable_value_fi, role
):
...
def test__accepted__not_owner__with_policy__success(
    api_client, monkeypatch, get_cu_fi, get_project_variable_value_fi, role
):
    r_cu = get_cu_fi(status=CUS.ACCEPT.value, role=role)
...
def test__response_data(
    ...
    data = new_project_variable_value_data(
        project=project, model_name=ProjectVariableValueModelName.RESERVATION
    )
    ...
    updated_instance = ProjectVariableValue.objects.get(id=instance.id)

    response_project_variable = response_instance.pop("project_variable")
    assert response_project_variable == {
        "id": updated_instance.project_variable.id,
        "key": updated_instance.project_variable.key,
        "kind": updated_instance.project_variable.kind,
    }
    assert response_instance.pop("model_id") == updated_instance.model_id == instance.model_id
    assert instance.model_id != data["model_id"]
    assert response_instance.pop("model_name") == updated_instance.model_name == instance.model_name
    assert instance.model_name != data["model_name"]
    assert response_instance.pop("value") == data["value"] == updated_instance.value
    assert instance.value != updated_instance.value
...
def test__deleted_project__fail(
    api_client, mock_policies_false, r_cu, get_project_fi, get_project_variable_value_fi
):
