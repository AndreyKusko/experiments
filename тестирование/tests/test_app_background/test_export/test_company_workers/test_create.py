...
from background.serializers.background_task import (
    OUTPUT_TYPE_MUST_BE_INT,
    TARGETS_AND_B_TASKS_COMPANY_NOT_MATCH,
)
...
from background.tasks.for_export.company_workers import export_company_workers_task
from background.serializers.for_export.geo_points import ExportGeoPointsSerializer
from tests.test_app_background.test_export.test_company_workers.constants import PATH, TASK_TYPE, MODEL_NAME
...
def test__accepted_owner__success(
    ...
    monkeypatch.setattr(export_company_workers_task, "delay", lambda *a, **k: None)
...
def test__not_accepted_owner__fail(
    api_client, monkeypatch, get_cu_owner_fi, new_background_task_data, get_cu_worker_fi, status
):
    monkeypatch.setattr(export_company_workers_task, "delay", lambda *a, **k: None)
...
def test__accepted_manager_with_policy__success(
    api_client, monkeypatch, r_cu, new_background_task_data, get_cu_worker_fi
):
    monkeypatch.setattr(export_company_workers_task, "delay", lambda *a, **k: None)
    monkeypatch.setattr(Policies, "has_object_policy", lambda *a, **k: True)
...
def test__accepted_manager_without_policy__fail(
    api_client, mock_policies_false, monkeypatch, r_cu, new_background_task_data, get_cu_worker_fi
):
    monkeypatch.setattr(export_company_workers_task, "delay", lambda *a, **k: None)
...
def test__not_accepted_manager_with_policy__fail(
    api_client, monkeypatch, get_cu_manager_fi, new_background_task_data, get_cu_worker_fi, status
):
    monkeypatch.setattr(export_company_workers_task, "delay", lambda *a, **k: None)
...
@pytest.mark.parametrize("r_cu", [pytest.lazy_fixture("cu_owner_fi")])
def test__another_company__fail(api_client, monkeypatch, r_cu, new_background_task_data, get_cu_worker_fi):
    monkeypatch.setattr(export_company_workers_task, "delay", lambda *a, **k: None)

    export_instance = get_cu_worker_fi()
    assert not BackgroundTask.objects.filter(company=r_cu.company, task_type=TASK_TYPE).exists()
    ...
    err_txt = TARGETS_AND_B_TASKS_COMPANY_NOT_MATCH.format(name="исполнителей", ids=export_instance.id)
    assert response.data == {"non_field_errors": [err_txt]}


@pytest.mark.parametrize("r_cu", [pytest.lazy_fixture("cu_owner_fi")])
def test__extra_keys__fails(api_client, monkeypatch, r_cu, new_background_task_data, get_cu_worker_fi):
    monkeypatch.setattr(export_company_workers_task, "delay", lambda *a, **k: None)
    extra_key = "qwe"
    export_instance = get_cu_worker_fi(company=r_cu.company)
...
def test__missed_keys__fails(
    api_client, monkeypatch, r_cu, new_background_task_data, get_cu_worker_fi, missed_key
):
    monkeypatch.setattr(export_company_workers_task, "delay", lambda *a, **k: None)
...
def test__response_data(
    ...
    created_instance = BackgroundTask.objects.get(id=response.data.pop("id"))
    ...
    assert response_instance.pop("model_name") == created_instance.model_name == MODEL_NAME

    assert response_instance.pop("task_type") == created_instance.task_type == TASK_TYPE
