...
URL = f"/{API_V1}/import-project-variable-values"


@pytest.mark.parametrize("r_cu", [pytest.lazy_fixture("cu_owner_fi")])
def test__method__fail(api_client, r_cu, get_background_task_fi: Callable):
...
@pytest.mark.parametrize("r_cu", [pytest.lazy_fixture("cu_owner_fi")])
def test__method__fail(api_client, r_cu):
    api_client.credentials(HTTP_AUTHORIZATION=get_authorization_token(r_cu.user))
