...
def __get_response(
    ...
    response = api_client.post(f"/api/v1/media-permissions/{PROJECT_QUESTIONNAIRE}/{model_id}/{object_id}/")
...
def test__user__success(api_client, mock_policies_false, r_u, get_project_questionnaire_fi):
...
@pytest.mark.parametrize("r_u", [pytest.lazy_fixture("user_fi")])
def test__familiar__fail(monkeypatch, api_client, r_u, get_user_fi, get_project_questionnaire_fi):
    object_id = "e0kj309230e0203"
    media_field = {TYPE: PHOTO, VALUES: [object_id], BASENAME: "123123123.jpeg"}
