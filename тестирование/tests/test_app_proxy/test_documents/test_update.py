...
__get_response = functools.partial(request_response_update, path="/api/v1/documents/")
...
def test__response_data(
    ...
    patch_return_data = docshub_document_response_fi(company_id=company.id)
    monkeypatch.setattr(requests, "patch", lambda *a, **k: MockResponse(data=patch_return_data))

    response = __get_response(
        api_client,
        instance_id=1,
        data={"title": "Тестовое факлашение", "description": "о", "file": file},
        user=r_cu.user,
...
def test__accepted_owner__success(
    ...
    monkeypatch.setattr(requests, "patch", lambda *a, **k: MockResponse(data=patch_return_data))
    data = {"title": "Тестовое факлашение", "description": "о", "file": file}
    response = __get_response(api_client, instance_id=1, data=data, user=r_cu.user)
    assert response.data


@pytest.mark.parametrize("r_cu", [pytest.lazy_fixture("cu_owner_fi")])
@pytest.mark.parametrize("file", [pytest.lazy_fixture("document__pdf_file_fi")])
def test__document__fail(monkeypatch, api_client, r_cu, file):
...
@pytest.mark.parametrize("r_cu", [pytest.lazy_fixture("cu_owner_fi")])
@pytest.mark.parametrize("file", [pytest.lazy_fixture("document__pdf_file_fi")])
def test__owner_system_file__fail(
    monkeypatch,
    api_client,
    docshub_document_response_fi: Callable,
    docshub_single_documents_response_fi: Callable,
...
def test__not_accepted_owner__fail(
    ...
    monkeypatch.setattr(requests, "get", lambda *a, **k: MockResponse(data=return_data))
    data = {"title": "Тестовое факлашение", "description": "о", "file": file}
    response = __get_response(api_client, 1, data, r_cu.user, status_codes=PermissionDenied)
...
def test__not_owner__fail(
    monkeypatch, api_client, docshub_single_documents_response_fi, get_cu_fi, file, role
):
